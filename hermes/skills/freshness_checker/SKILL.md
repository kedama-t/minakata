...
4. `freshness_rank` が `stale` / `very_stale` の記事に対して **`minakata.report_progress({ agent_name: "freshness_checker", phase: "リフレッシュ投入", detail: "記事 …" + article_id末尾8文字 })`** を呼んでから `enqueue_task(type="refresh", priority="scheduled", payload={article_id}, dedup_key="refresh:{article_id}:{YYYY-MM-DD}")`
